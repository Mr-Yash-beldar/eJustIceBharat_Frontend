import * as React from 'react';

// Define the types for the props, extending SVGProps to accept all standard SVG attributes
interface ScreenShareIconProps extends React.SVGProps<SVGSVGElement> {
  fillcolor?: string; // Optional 'fillcolor' prop to allow customization of the icon color
}

const ScreenShareIcon: React.FC<ScreenShareIconProps> = (props) => (
  <svg
    width={24}
    height={24}
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props} // Spread any additional props that might be passed to the component
  >
    <path
      d="M20 18c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2H4c-1.1 0-2 .9-2 2v10a2 2 0 0 0 2 2H1c-.55 0-1 .45-1 1s.45 1 1 1h22c.55 0 1-.45 1-1s-.45-1-1-1h-3Zm-7-3.53v-2.19c-2.78 0-4.61.85-6 2.72.56-2.67 2.11-5.33 6-5.87V7l3.61 3.36c.21.2.21.53 0 .73L13 14.47Z"
      fill={props.fillcolor || '#000'} // Default fill color is black if no 'fillcolor' is provided
    />
  </svg>
);

export default ScreenShareIcon;
