...
interface ParticipantGridProps {
  ...
  isPresenting: boolean; // Whether a participant is presenting
}
...
function ParticipantGrid({ participantIds, isPresenting }: ParticipantGridProps): JSX.Element {
  ...
  return (
    ...
    >
      <div className="flex flex-col w-full h-full">
        {Array.from({ length: Math.ceil(participantIds.length / perRow) }, (_, i) => {
          return (
            <div
              key={`participant-${i}`}
            ...
            >
              {participantIds.slice(i * perRow, (i + 1) * perRow).map((participantId) => {
                return (
                  <div
